...
def optimizer_kwargs(cfgs):
    ...
    if getattr(cfgs, 'opt_eps', None) is not None:
        kwargs['eps'] = cfgs.opt_eps
    if getattr(cfgs, 'opt_betas', None) is not None:
        kwargs['betas'] = cfgs.opt_betas
    if getattr(cfgs, 'opt_args', None) is not None:
        kwargs.update(cfgs.opt_args)
    return kwargs
