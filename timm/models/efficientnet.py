...
def _create_effnet(variant, pretrained=False, **kwargs):
    features_only = False
    model_cls = EfficientNet      #模型构建类函数名
    kwargs_filter = None
    if kwargs.pop('features_only', False):
        features_only = True
        kwargs_filter = ('num_classes', 'num_features', 'head_conv', 'global_pool')
        model_cls = EfficientNetFeatures   #模型构建类函数名
    model = build_model_with_cfg(
        model_cls, variant, pretrained,
        default_cfg=default_cfgs[variant],  #模型预训练下载配置参数
...
def _gen_efficientnet(variant, channel_multiplier=1.0, depth_multiplier=1.0, pretrained=False, **kwargs):
    ...
    print('---------------------------_gen_efficientnet kwargs-----------------------------')
    print(kwargs)
    round_chs_fn = partial(round_channels, multiplier=channel_multiplier)  #获得通道放缩函数入口
    model_kwargs = dict(
        block_args=decode_arch_def(arch_def, depth_multiplier),  #首先根据深度放缩系数获得网络深度架构
        num_features=round_chs_fn(1280),   #最后一层通道数
        stem_size=32,
        round_chs_fn=round_chs_fn,
        act_layer=resolve_act_layer(kwargs, 'swish'),    ##SiLU
        norm_layer=kwargs.pop('norm_layer', None) or partial(nn.BatchNorm2d, **resolve_bn_args(kwargs)),
        **kwargs,
    )
    print('---------------------------_gen_efficientnet model_kwargs-----------------------------')
    print(model_kwargs)
