...
def conv2d_cx(cx, w_in, w_out, k, *, stride=1., groups=1., bias=False):
    ...
    assert k % 2 == 1, "Only odd size kernels supported to avoid padding issues." #内核大小为奇数，偶数内核大小不能实现same conv padding对称填充
    h, w, flops, params, acts = cx["h"], cx["w"], cx["flops"], cx["params"], cx["acts"]
    h, w = (h - 1.0) // stride + 1.0, (w - 1.0) // stride + 1.0  #由于conv的Padding设置为(k-1)//2，所以输出维度变为该式
...
def norm2d_cx(cx, w_in):
    ...
    h, w, flops, params, acts = cx["h"], cx["w"], cx["flops"], cx["params"], cx["acts"]
    params += 2 * w_in ##数据归一化处理，包含两个可学习的参数（每一个通道共用一个参数）
    return {"h": h, "w": w, "flops": flops, "params": params, "acts": acts}
