...
class TrainMeter(object):
    ...
    def update_summary(metrics, filename, write_header=False, log_wandb=False):
        rowd = OrderedDict()
        rowd.update(metrics)
        if log_wandb:
            wandb.log(rowd)
    ...
    def log_epoch_stats(self, cur_epoch, filename):
        stats = self.get_epoch_stats(cur_epoch)
        logger.info(logging.dump_log_data(stats, self.phase + "_epoch"))
        self.update_summary(metrics=stats, filename=filename)
...
class TestMeter(object):
    ...
    def update_summary(self, metrics, filename, write_header=False, log_wandb=False):
        rowd = OrderedDict()
        rowd.update(metrics)
        if log_wandb:
            wandb.log(rowd)
    ...
    def log_epoch_stats(self, cur_epoch,filename):
        stats = self.get_epoch_stats(cur_epoch)
        logger.info(logging.dump_log_data(stats, self.phase + "_epoch"))
        self.update_summary(metrics=stats, filename=filename,write_header=cur_epoch is None)
