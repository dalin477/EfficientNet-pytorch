...
def train_epoch(loader, model, ema, loss_fun, optimizer, scaler, meter, cur_epoch,filename):
    ...
    for cur_iter, (inputs, labels) in enumerate(loader):
        ...
        meter.iter_tic()
    # Log epoch stats
    meter.log_epoch_stats(cur_epoch,filename)


@torch.no_grad()
def test_epoch(loader, model, meter, cur_epoch,filename):
...
def train_model():
    ...
    filename = os.path.join(cfg.OUT_DIR, 'summary.csv')
    # Perform the training loop
    logger.info("Start epoch: {}".format(start_epoch + 1))
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        # Train for one epoch
        params = (train_loader, model, ema, loss_fun, optimizer, scaler, train_meter)
        train_epoch(*params, cur_epoch,filename)
        # Compute precise BN stats
        if cfg.BN.USE_PRECISE_STATS:
            net.compute_precise_bn_stats(model, train_loader)
            net.compute_precise_bn_stats(ema, train_loader)
        # Evaluate the model
        test_epoch(test_loader, model, test_meter, cur_epoch,filename)
        test_epoch(test_loader, ema, ema_meter, cur_epoch,filename)
...
def test_model():
    ...
    model = setup_model()
    # Load model weights
    # test_weights = get_weights_file()
    test_weights = os.path.join(cfg.OUT_DIR, cfg.TEST.WEIGHTS)
    filename = os.path.join(cfg.OUT_DIR, 'summary.csv')
    ...
    test_meter = meters.TestMeter(len(test_loader))
    # Evaluate the model
    test_epoch(test_loader, model, test_meter, 0,filename)
